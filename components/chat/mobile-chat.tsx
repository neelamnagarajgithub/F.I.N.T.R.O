    ...
    <Sheet open={isOpen} onOpenChange={handleOpenChange} defaultOpen={false}>
      ...
      <SheetTrigger asChild>
        <Button size="xl" className="pl-3 fixed gap-4 bottom-4 right-4 z-40 md:hidden">
          {/* Status Indicator */}
          <ChatStatusIndicator isExpanded={false} hasUnreadMessages={hasNewMessages} unreadCount={totalUnreadCount} />

          {/* Title */}
          <span>{hasNewMessages ? `New Message${totalUnreadCount > 1 ? "s" : ""}` : "Fintro Copilot"}</span>
        </Button>
      </SheetTrigger>
      ...
      <SheetContent side="bottom" className="h-[85vh] p-0 md:hidden" closeButton={false}>
        <SheetHeader className="sr-only">
          <SheetTitle>Fintro Copilot</SheetTitle>
        </SheetHeader>
